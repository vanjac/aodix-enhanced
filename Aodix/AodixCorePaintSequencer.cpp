			...
			if(pe->pat==user_pat && pe->trk>=user_trk_offset && pe->trk<last_visible_track)
			{
				...
				if(e_y<seq_area_b && e_b>seq_area_y)
				{
					...
					if(pe->typ==0)
					{
						...
						paint_event(hdc,e_x,e_y,TRACK_WIDTH,e_h,0x00808080+clipped_vel*0x00010101,color+0x202020,buf_a);
						...
						if(pe->trk==user_trk)
						{
							...
							if(pr_ni>=0 && pr_ni<pr_vn)
							{
								...
								arg_gdi_paint_solid_rect(hdc,TRACK_WIDTH+5+pr_ex,e_y+1,user_pr_note_width-2,e_h-2,color+(clipped_vel/2)*0x010101);
							}
						}
					}
